...
export var emojiRequest = function (provider, options) {
    var _a = provider.getRatio, getRatio = _a === void 0 ? getPixelRatio : _a, serviceConfig = __rest(provider, ["getRatio"]);
    var scaleQueryParams = calculateScale(getRatio);
    var _b = options || {}, _c = _b.queryParams, queryParams = _c === void 0 ? {} : _c, otherOptions = __rest(_b, ["queryParams"]);
    var requestOptions = __assign(__assign({}, otherOptions), { queryParams: __assign(__assign({}, scaleQueryParams), queryParams) });
...
export var denormaliseSkinEmoji = function (emoji, meta) {
    ...
    return skinEmoji.map(function (skin) {
        var representation = skin.representation, altRepresentations = skin.altRepresentations, other = __rest(skin, ["representation", "altRepresentations"]);
        return __assign({ baseId: baseId, representation: denormaliseServiceRepresentation(representation, meta), altRepresentation: denormaliseServiceAltRepresentation(altRepresentations, meta) }, other);
    });
};
/**
...
export var denormaliseEmojiServiceResponse = function (emojiData) {
    var emojis = emojiData.emojis.map(function (emoji) {
        ...
        var representation = emoji.representation, skinVariations = emoji.skinVariations, altRepresentations = emoji.altRepresentations, trimmedServiceDesc = __rest(emoji, ["representation", "skinVariations", "altRepresentations"]);
        var response = __assign(__assign({}, trimmedServiceDesc), { representation: newRepresentation, skinVariations: newSkinVariations });
