...
function createRule() {
    ...
    return function media(State) {
        ...
        var createInlineTokens = function (str, openingTokens, closingTokens) {
            ...
            return __spread(openingTokens, [inlineBefore], closingTokens);
        };
        var processedTokens = [];
        var newTokens = State.tokens.reduce(function (tokens, token, i, arr) {
            if (token.type === 'inline' && regx.test(token.content)) {
                ...
                matches.forEach(function (match) {
                    ...
                    subTree_1 = __spread(subTree_1, createInlineTokens(contentBefore, openingTokens_1, closingTokens_1), createMediaTokens(match));
                });
                if (inlineContentStack_1.length) {
                    subTree_1 = __spread(subTree_1, createInlineTokens(inlineContentStack_1, openingTokens_1, closingTokens_1));
                }
                processedTokens = __spread(processedTokens, closingTokens_1.map(function (c) { return c.type; }));
                tokens = __spread(tokens.slice(0, cursor), subTree_1);
            }
