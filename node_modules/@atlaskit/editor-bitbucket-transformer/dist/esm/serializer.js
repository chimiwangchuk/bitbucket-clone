...
var MarkdownSerializerState = /** @class */ (function (_super) {
    __extends(MarkdownSerializerState, _super);
    ...
    MarkdownSerializerState.prototype.renderInline = function (parent) {
        ...
        var progress = function (node, _, index) {
            ...
            if (node &&
                ...
                var _a = __read(/^(\s*)(.*?)(\s*)$/m.exec(node.text), 4), lead = _a[1], inner = _a[2], trail = _a[3];
                leading += lead;
                trailing = trail;
                if (lead || trail) {
...
var MarkdownSerializer = /** @class */ (function (_super) {
    __extends(MarkdownSerializer, _super);
...
var editorNodes = {
    ...
    bulletList: function (state, node) {
        for (var i = 0; i < node.childCount; i++) {
            var child = node.child(i);
            state.render(child, node, i);
        }
    },
    orderedList: function (state, node) {
        for (var i = 0; i < node.childCount; i++) {
            var child = node.child(i);
            state.render(child, node, i);
        }
    ...
    emoji: function (state, node) {
        state.write(node.attrs.shortName);
    },
};
export var nodes = __assign(__assign({}, editorNodes), tableNodes);
export var marks = {
