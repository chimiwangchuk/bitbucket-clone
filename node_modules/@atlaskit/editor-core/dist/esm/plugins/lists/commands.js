...
import { hasParentNodeOfType, findPositionOfNodeBefore, findParentNodeOfTypeClosestToPos, } from 'prosemirror-utils';
import { hasVisibleContent, isNodeEmpty } from '../../utils/document';
import { filter, isEmptySelectionAtStart, isFirstChildOfParent, findCutBefore, insertContentDeleteRange, walkNextNode, isEmptySelectionAtEnd, } from '../../utils/commands';
...
export var deleteKeyCommand = function (state, dispatch) {
    var tr = state.tr;
    ...
    if (isEmptySelectionAtEnd(state) &&
        ...
        var _a = walkNextNode($head), $next = _a.$pos, foundNode = _a.foundNode;
        var nextGrandParent = $next.node(-1);
        if (foundNode) {
            if ($next.parent && $next.parent.type.name === 'paragraph') {
                //Next is a paragraph
                var content = $next.parent.content;
                insertContentDeleteRange(tr, function (tr) { return tr.doc.resolve($head.pos); }, [
                    [content, $head.pos],
                ], [
                    [$next.before(), $next.before() + $next.parent.nodeSize],
                ]);
                if (dispatch) {
                    dispatch(tr);
                }
            ...
            else if (headGreatGrandParent === nextGrandParent &&
                ...
                var content = $next.parent.firstChild.content;
                var childrenContent = $next.parent.content.cut($next.parent.firstChild.nodeSize);
                insertContentDeleteRange(tr, function (tr) { return tr.doc.resolve($head.pos); }, [
                    [content, $head.pos],
                    [childrenContent, $head.after()],
                ], [
                    [$next.before(), $next.before() + $next.parent.nodeSize],
                ]);
                if (dispatch) {
...
filter([
    ...
    canOutdent,
], baseCommand.chainCommands(deletePreviousEmptyListItem, outdentList(INPUT_METHOD.KEYBOARD))), 
...
export function outdentList(inputMethod) {
    if (inputMethod === void 0) { inputMethod = INPUT_METHOD.KEYBOARD; }
    return function (state, dispatch) {
        var listItem = state.schema.nodes.listItem;
...
export function indentList(inputMethod) {
    if (inputMethod === void 0) { inputMethod = INPUT_METHOD.KEYBOARD; }
    return function (state, dispatch) {
        var listItem = state.schema.nodes.listItem;
        if (isInsideListItem(state)) {
...
export function toggleListCommand(listType) {
    return function (state, dispatch, view) {
        ...
        if (isInsideList(state, listType) && isSameListTypeSelected) {
            ...
            var tr = sanitiseSelectionMarksForWrapping(state, listNodeType);
            if (tr && dispatch) {
                dispatch(tr);
                state = view.state;
            }
