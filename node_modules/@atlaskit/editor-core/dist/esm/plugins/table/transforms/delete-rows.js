...
export var deleteRows = function (rect, isHeaderRowRequired) {
    ...
    return function (tr) {
                ...
                if (rowsToDelete.indexOf(rowIndex) === -1 && !seen[cellPos]) {
                    ...
                    if (cellsInRow.indexOf(cellPos) > -1) {
                        ...
                        if (overlappingRows_1 > 0) {
                            var newCell = cell.type.createChecked(__assign(__assign({}, cell.attrs), { rowspan: cell.attrs.rowspan - overlappingRows_1 }), cell.content, cell.marks);
        ...
        if (fixedTable === null) {
            return setMeta({ type: 'DELETE_ROWS', problem: 'REMOVE_EMPTY_COLUMNS' })(tr);
        }
        var cursorPos = getNextCursorPos(newTable, rowsToDelete);
        return setMeta({ type: 'DELETE_ROWS' })(tr
...
function getNextCursorPos(table, deletedRows) {
    var minRow = Math.min.apply(Math, __spread(deletedRows));
