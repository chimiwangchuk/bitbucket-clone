...
export function mergeCells(tr) {
    ...
    for (var rowIndex = 0; rowIndex < map.height; rowIndex++) {
        ...
        for (var colIndex = 0; colIndex < map.width; colIndex++) {
            ...
            if (colIndex === rect.left && rowIndex === rect.top) {
                ...
                if (rowspan < 1) {
                    return setMeta({ type: 'MERGE_CELLS', problem: 'NEGATIVE_ROWSPAN' })(tr);
                }
                // update colspan and rowspan of the merged cell to span the selection
                var attrs = addColSpan(__assign(__assign({}, cell.attrs), { rowspan: rowspan }), cell.attrs.colspan, rect.right - rect.left - cell.attrs.colspan);
        ...
        if (rowCells.length) {
            ...
            for (var i = rows.length - 1; i >= 0; i--) {
                ...
                for (var j = 0; j < prevRow.childCount; j++) {
                    ...
                    var rowspan = cell.attrs.rowspan;
                    if (rowspan && rowspan + i - 1 >= rows.length) {
                        ...
                        cells.push(cell.type.createChecked(__assign(__assign({}, cell.attrs), { rowspan: rowspan - 1 }), cell.content, cell.marks));
                    }
                    else {
                        cells.push(cell);
                    }
    ...
    if (fixedTable === null) {
        return setMeta({ type: 'MERGE_CELLS', problem: 'REMOVE_EMPTY_COLUMNS' })(tr);
    }
    return setMeta({ type: 'MERGE_CELLS' })(tr
...
function addColSpan(attrs, pos, span) {
    ...
    var newAttrs = __assign(__assign({}, attrs), { colspan: (attrs.colspan || 1) + span });
        ...
        for (var idx in cols) {
            ...
            if (cell && rect.top === rowIndex) {
                if (minColSpans[colIndex] > 1) {
                    ...
                    var newCell = cell.type.createChecked(__assign(__assign({}, cell.attrs), { colspan: colspan, colwidth: colwidth ? colwidth.slice(0, colspan) : null }), cell.content, cell.marks);
