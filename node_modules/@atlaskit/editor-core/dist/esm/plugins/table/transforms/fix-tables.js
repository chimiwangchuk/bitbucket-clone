...
import { calculateColumnWidth, getCellsRefsInColumn, contentWidth, getLayoutSize, } from '../pm-plugins/table-resizing/utils';
...
export var removeExtraneousColumnWidths = function (node, basePos, tr) {
    ...
    tr = replaceCells(tr, node, basePos, function (cell) {
        ...
        if (colwidth && colwidth.length > colspan) {
            hasProblems = true;
            return cell.type.createChecked(__assign(__assign({}, cell.attrs), { colwidth: colwidth.slice(0, colspan) }), cell.content, cell.marks);
        }
        return cell;
    });
    if (hasProblems) {
...
export var fixAutoSizedTable = function (view, tableNode, tableRef, tablePos, opts) {
    ...
    tr = replaceCells(tr, tableNode, tablePos, function (cell, _rowIndex, colIndex) {
        ...
        return cell.type.createChecked(__assign(__assign({}, cell.attrs), { colwidth: newColWidths.length ? newColWidths : null }), cell.content, cell.marks);
    });
    // clear autosizing on the table node
    return tr
        .setNodeMarkup(tablePos, undefined, __assign(__assign({}, tableNode.attrs), { layout: tableLayout, __autoSize: false }))
